...
async function pollNotifications() {
  ...
  if (data?.success && data.notifications.length > lastNotifCount) {
    ...
    if (window.location.hash === '#notifications' && typeof window.loadNotifications === 'function') {
...
window.loadNotifications = async function() {
  ...
  try {
    const data = await api('/api/user/notifications');
    if (!data?.success) {
      container.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-muted);">Failed to load notifications.</div>';
      return;
    }
    const notifs = data.notifications;
    if (!notifs || notifs.length === 0) {
      container.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-muted);">No new notifications.</div>';
      return;
    }
    container.innerHTML = notifs.map(n => {
      const dateStr = new Date(n.createdAt).toLocaleDateString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
      return `
        <div class="notif-item ${n.read ? '' : 'unread'}">
          <div class="notif-dot"></div>
          <div>
            <div style="font-size:14px;font-weight:500;">${n.title}</div>
            <div class="text-sm mt-4">${n.message}</div>
            <div class="text-xs" style="margin-top:6px;">${dateStr}</div>
          </div>
        </div>
      `;
    }).join('');
    ...
    container.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-muted);">Error loading notifications.</div>';
  }
};

window.markAllNotificationsRead = async function() {
  try {
    const data = await api('/api/user/notifications/read-all', { method: 'POST' });
    if (data?.success) {
      showAlert('All notifications marked as read.', 'success', 'ri-check-double-line', 'Notifications');
      window.loadNotifications();
    } else {
      showAlert(data?.error || 'Failed to mark as read.', 'error', 'ri-close-line', 'Error');
    }
  } catch (err) {
    console.error('Error in markAllNotificationsRead:', err);
    showAlert('Error connecting to server.', 'error', 'ri-close-line', 'Error');
  }
};
