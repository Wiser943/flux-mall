...
if (toggler) {
  toggler.addEventListener('change', function() {
    if (this.checked) { document.body.classList.add('dark'); setTheme('dark'); }
    else              { document.body.classList.remove('dark'); setTheme('light'); }
  });
}
...
window.approveDeposit = async (id, userId, amount) => {
  ...
  if (data?.success) { showToast('Deposit approved and user credited!','success'); loadAnalytics(); }
...
window.toggleBanStatus = async (uid, isBanned) => {
  ...
  if (!data?.success) { alert('Error updating ban status.'); renderApiUsers(); }
};
...
function clearSearch() {
  ...
  if (si) { si.value = ''; document.getElementById('searchClear')?.classList.remove('vis'); }
  applyFilters();
}
...
function openDetail(id) {
  ...
  if (dp) { dp.classList.remove('hidden'); dp.classList.add('mobile-open'); }
...
function closeDetail() {
  activeUserId = null;
  const dp = document.getElementById('detailPane');
  if (dp) { dp.classList.add('hidden'); dp.classList.remove('mobile-open'); }
  renderTable();
}
...
function submitAddUser() {
  ...
  const newUser  = {
    ...
    lastSeen:    'Just now', referredBy:null, device:'Unknown', ip:'—',
  };
  UM_USERS.unshift(newUser);
  closeModal(); updateUMStats(); applyFilters();
...
function bulkVerify() {
  ...
  clearSelection(); updateUMStats(); applyFilters();
...
function submitBulkBan() {
  ...
  clearSelection(); closeModal(); updateUMStats(); applyFilters();
...
function onToggle(feature, checked) {
  if (feature === 'maintenance' && checked) {
    ...
    showConfirm({
      ...
      type: 'danger', yesLabel: 'Enable Maintenance',
      onYes: () => {
  ...
  if (feature === 'withdrawals' && !checked) {
    ...
    showConfirm({
      ...
      type: 'warning', yesLabel: 'Disable Withdrawals',
      onYes: () => {
  ...
  if (feature === 'register' && !checked) {
    ...
    showConfirm({
      ...
      type: 'warning', yesLabel: 'Stop Registrations',
      onYes: () => {
...
async function saveFeatureState(feature, value) {
  // await api('/api/admin/settings/toggle', { method:'POST', body:JSON.stringify({ feature, value }) });
...
function confirmDiscard() {
  showConfirm({
    ...
    type: 'warning', yesLabel: 'Discard Changes',
...
window.addEventListener('beforeunload', (e) => {
  if (unsavedChanges > 0) { e.preventDefault(); e.returnValue = ''; }
});
...
function saveAllSettings() {
  ...
  const payload = {
    siteName:     g('siteName'),
    siteTagline:  g('siteTagline'),
    siteAbout:    g('siteDesc'),
    whatsappLink: g('whatsappLink'),
    telegramLink: g('telegramLink'),
    dailyCheckInAmount: g('checkinBonus'),
    referralPercents: [g('refL1'), g('refL2'), g('refL3')].map(Number),
    minDeposit:   g('minDeposit'),
    maxDeposit:   g('maxDeposit'),
    minWithdraw:  g('minWithdraw'),
    withdrawFee:  g('withdrawFee'),
  ...
  // await api('/api/admin/settings/config', { method:'PUT', body:JSON.stringify(payload) });
...
function resetDefaults() {
  showConfirm({
    ...
    type: 'warning', yesLabel: 'Yes, Reset Everything',
    onYes: () => {
      const s = (id, v) => { const el = document.getElementById(id); if (el) el.value = v; };
      s('siteName','Fluxmall Exchange'); s('siteTagline','Setting Standards');
      s('checkinBonus','50'); s('refL1','5'); s('refL2','3'); s('refL3','1');
      s('minDeposit','1000'); s('maxDeposit','500000');
      s('minWithdraw','500'); s('withdrawFee','2');
...
function clearSettingsSearch() {
  ...
  if (input) { input.value = ''; handleSettingsSearch(''); input.focus(); }
...
function saveBrandAssets() { showToast('Brand assets saved','success'); markChanged(); }
...
window.applyPreset = (mode, primary, secondary) => {
  const s = (id, v) => { const el = document.getElementById(id); if (el) el.value = v; };
  s('themeMode', mode); s('primaryColor', primary); s('secondaryColor', secondary);
  saveThemeConfig();
};
...
window.saveThemeConfig = async (e) => {
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.innerText = 'Save Theme'; }
  }
};
...
if (rulesForm) {
  ...
  rulesForm.onsubmit = async (e) => {
    ...
    try {
      ...
      if (btn) { btn.disabled = false; btn.innerText = 'Save Config'; }
    }
  };
}
...
window.openBindBankModal = async () => {
  ...
  showModal({
    id: 'createBankModal', title: 'Payments Configuration',
    content: `
...
function handleChatSearch(query) {
  ...
  if (q) users = CHAT_USERS.filter(u => u.name.toLowerCase().includes(q) || u.lastMsg.toLowerCase().includes(q) || u.email.toLowerCase().includes(q));
...
function applyChatFilter(users) {
  switch (currentFilter) {
    case 'unread':   return users.filter(u => u.unread > 0);
    case 'open':     return users.filter(u => u.status === 'open');
    case 'resolved': return users.filter(u => u.status === 'resolved');
    case 'pinned':   return users.filter(u => u.pinned);
...
function ctxAction(action) {
  ...
  switch (action) {
    case 'reply':
      startReply(ctxMsgId, ctxMsgText); break;
    case 'copy':
      navigator.clipboard.writeText(ctxMsgText).then(() => showToast('Copied!','success')); break;
    case 'forward':
      showToast('Forward: coming soon','info'); break;
    case 'star':
      showToast('Message starred ⭐','success'); break;
    case 'delete':
      if (chatActiveUserId && ctxMsgId) {
        const msgs = CHAT_MESSAGES[chatActiveUserId];
        const idx  = msgs.findIndex(m => m.id === ctxMsgId);
        if (idx > -1) { msgs.splice(idx,1); renderMessages(chatActiveUserId); showToast('Message deleted','info'); }
      }
      break;
  }
}
...
function resolveSession() {
  ...
  if (ia) { ia.style.opacity = '0.5'; ia.style.pointerEvents = 'none'; }
...
function reopenSession() {
  ...
  const ia = document.getElementById('inputArea');
  if (ia) { ia.style.opacity = ''; ia.style.pointerEvents = ''; }
...
window.executeChatCtx = function(userId, actionIdx) {
  ...
  if (!menu || !user) return;
  const actions = [
    () => { user.pinned = !user.pinned; handleChatSearch(''); showToast(user.pinned?'Chat pinned':'Chat unpinned','info'); },
    () => { user.muted  = !user.muted;  handleChatSearch(''); showToast(user.muted?'Chat muted':'Chat unmuted','info'); },
    () => { user.unread = 1; handleChatSearch(''); showToast('Marked as unread','info'); },
    () => { user.status = 'resolved'; handleChatSearch(''); showToast('Resolved','success'); },
  ];
  actions[actionIdx]?.();
...
window.adminShowEmojiBar  = (e, barId) => { e.preventDefault(); adminHideAllEmojiBars(); const b = document.getElementById(barId); if (b) b.style.display = 'flex'; };
...
async function loadChatSettings() {
  ...
  sc('cs_available',s.available!==false); sc('cs_sound',s.sound!==false); sc('cs_allowImages',s.allowImages!==false);
  sc('cs_requireVerified',!!s.requireVerified); sc('cs_officeHoursEnabled',!!s.officeHours?.enabled);
  sv('cs_autoReply',s.autoReply); sv('cs_open',s.officeHours?.open||9); sv('cs_close',s.officeHours?.close||18);
  sv('cs_offlineMsg',s.officeHours?.offlineMsg); sv('cs_autoClose',s.autoClose||48); sv('cs_charLimit',s.charLimit||500);
...
window.saveChatSettings = async () => {
  ...
  if (data?.success) { adminSoundEnabled = body.sound; showToast('Chat settings saved!','success'); }
...
window.toggleChatSearch = () => {
  ...
  if (!isVisible) document.getElementById('chatSearchInput')?.focus();
    ...
    const si = document.getElementById('chatSearchInput'); if (si) si.value = '';
    const sr = document.getElementById('chatSearchResults'); if (sr) sr.textContent = '';
...
window.searchChatMessages = (query) => {
  ...
  if (resultsEl) { resultsEl.textContent = matchCount>0?`${matchCount} result${matchCount>1?'s':''} found`:'No results found'; resultsEl.style.color = matchCount>0?'#10ac84':'#e74c3c'; }
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Escape') {
    // Settings search
    const si = document.getElementById('settingsSearch');
    if (document.activeElement === si) { clearSettingsSearch(); si.blur(); return; }
    // Confirm modal
    closeConfirm();
    // Ctx menus
    hideCtx();
