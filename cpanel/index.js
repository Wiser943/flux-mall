...
function switchPageByHash() {
  ...
  if (tabName) {
    // changed '=' to '*=' so it matches a substring
...
async function loadSettings() {
  try {
    ...
    // Fetch quick deposit amounts from dedicated API
    const depData = await api('/api/admin/deposit-amounts');
    if (depData?.success && Array.isArray(depData.amounts)) {
      depositAmounts.length = 0;
      depData.amounts.forEach(item => depositAmounts.push(item.amount));
    }
...
function renderPagination(prefix, total, page, onPage, limit = 5) {
...
function paginate(arr, page, limit = 5) {
...
// Implement window.simulateTyping
window.simulateTyping = function() {
  if (activeSessionId) {
    api('/api/admin/chat/typing', { method: 'POST', body: JSON.stringify({ sessionId: activeSessionId }) });
  }
};
...
function applyFilter(sessions) {
  switch (currentFilter) {
    case 'unread':
      return sessions.filter(s => s.unreadAdmin > 0);
    case 'open':
...
function chatItemCtx(e, sessionId, sessionData) {
  ...
  const actions = [
    ...
    {
      label: sessionData?.status === 'ended' ? 'Reopen chat' : 'Resolve chat',
      fn: () => {
...
function _closeChatUI() {
  ...
  const hash = window.location.hash || '';
  const basePath = hash.split('?')[0].replace(/^#/, '');
  history.replaceState(null, '', window.location.pathname + '#' + basePath);
  
  
  // FIX: Explicitly check for sideBar to avoid "Invalid left-hand side" errors
...
window.addEventListener('popstate', () => {
  ...
  chatId ? openChat(chatId) : _closeChatUI();
});
