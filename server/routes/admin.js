...
// HELPERS
// ═══════════════════════════════════════════════════════════
async function triggerKorapayPayout({ amount, bankCode, accountNumber, accountName, reference, narration }) {
  const proxyUrl = process.env.PROXY_URL;
  const proxySecret = process.env.PROXY_SECRET;
  
  // TEMP DEBUG — remove after fixing
  console.log('PROXY_URL:', proxyUrl);
  console.log('PROXY_SECRET set:', !!proxySecret);
  
  if (!proxyUrl) throw new Error('PROXY_URL is not set');
  if (!proxySecret) throw new Error('PROXY_SECRET is not set');
  ...
  console.log('Vercel response body:', responseText);
  ...
  if (!response.ok || !data.status) {
    throw new Error(data.message || `Korapay error: ${response.status}`);
  }
...
async function handleReferralCommission(depositorUid, depositAmount, tid) {
  try {
    ...
    // Flat referral bonus — fires once on first deposit
    if (!user.referralCompleted && user.emailVerified && user.referrerId) {
      const referralBonus = config?.value?.referralBonus || 840;

      await User.findByIdAndUpdate(user.referrerId, { $inc: { ib: referralBonus } });
      await User.findByIdAndUpdate(depositorUid, { referralCompleted: true });
    ...
    // Percentage commission — fires once per depositor
...
router.get('/analytics', requireAdmin, async (req, res) => {
  try {
    ...
    let successV = 0, pendingV = 0, sCount = 0, pCount = 0, dCount = 0, withdrawSuccessV = 0;
    ...
    withdrawals.forEach(w => withdrawSuccessV += w.netAmount || w.amount);
...
// NOTE: /tasks/submissions must be defined BEFORE /tasks/:id to avoid route conflict
...
router.put('/withdrawals/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (status === 'success') {
      // Bank details: bankName stores the bank CODE (e.g. "058" for GTBank)
      const bankCode      = withdrawal.bankDetails?.bankName;
      ...
      if (!bankCode || !accountNumber || !accountName) {
        return res.status(400).json({
          error: `Missing bank details — bankCode: ${bankCode}, accountNumber: ${accountNumber}, accountName: ${accountName}`,
        });
      }
      ...
      try {
        ...
        return res.status(502).json({
          error: `Korapay payout failed: ${koraErr.message}. Withdrawal marked as "failed". User balance NOT changed — investigate and retry or decline manually.`,
        });
      }
...
// NOTE: /activity/clear-all must be defined BEFORE /activity/:userId to avoid route conflict
...
router.get('/activity', requireAdmin, async (req, res) => {
  try {
    ...
    res.json({ success: true, activity, total, page: Number(page), pages: Math.ceil(total / Number(limit)) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/activity/:userId', requireAdmin, async (req, res) => {
  try {
    const activity = await Activity.find({ userId: req.params.userId }).sort({ createdAt: -1 });
...
router.put('/settings/:key', requireAdmin, async (req, res) => {
  try {
    ...
    if (key === 'config') {
      // Sync quick-select deposit amounts if provided
      if (incomingData.depositAmounts !== undefined && Array.isArray(incomingData.depositAmounts)) {
        await DepositAmt.deleteMany({});
        const insertData = incomingData.depositAmounts.map(a => ({ amount: Number(a) }));
        await DepositAmt.insertMany(insertData);
      }

      // Sync feature flags
      const features = {};
      const featureKeys = ['deposits', 'withdrawals', 'shares', 'referral', 'spin', 'register'];
      featureKeys.forEach(f => {
        if (incomingData[f] !== undefined) {
          features[f] = incomingData[f];
        }
      });
      if (Object.keys(features).length > 0) {
        const featUpdate = {};
        for (const f in features) {
          featUpdate[`value.${f}`] = features[f];
        }
        await Settings.findOneAndUpdate({ key: 'features' }, { $set: featUpdate }, { upsert: true });
      }
    }
    ...
    await Settings.findOneAndUpdate({ key }, { $set: updateObj }, { upsert: true, new: true });
...
router.post('/settings/toggle', requireAdmin, async (req, res) => {
  try {
    const { feature, value } = req.body;
    if (!feature) return res.status(400).json({ error: 'Feature name required.' });

    const updateObj = {};
    updateObj[`value.${feature}`] = value;
    ...
    res.json({ success: true, message: `Feature toggle "${feature}" set to ${value}.` });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/settings/maintenance', requireAdmin, async (req, res) => {
  try {
    ...
    await Settings.findOneAndUpdate(
      { key: 'maintenance' },
      { key: 'maintenance', value: { enabled: !!value } },
      { upsert: true, new: true }
    );
    res.json({ success: true, message: `Maintenance mode set to ${value}.` });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.put('/settings/apikeys', requireAdmin, async (req, res) => {
  try {
    const { imgbb, korapay_public } = req.body;
    // korapay_secret is stored in .env, NOT in the DB — never store secrets in DB
    await Settings.findOneAndUpdate(
      { key: 'apikeys' },
      { key: 'apikeys', value: { imgbb, korapay_public } },
      { upsert: true, new: true }
    );
    res.json({ success: true });
  } catch (err) {
...
router.get('/purchased-shares', requireAdmin, async (req, res) => {
  try {
    const investments = await PurchasedShare.find().sort({ createdAt: -1 }).populate('userId', 'username email');
...
router.get('/chat/sessions', requireAdmin, async (req, res) => {
  try {
    const sessions = await ChatSession.find()
      ...
      .populate('userId', 'username email ib status emailVerified createdAt refPoints referrerId');
...
// NOTE: /campaigns/settings must be defined BEFORE /campaigns/:id to avoid route conflict
...
router.put('/campaigns/settings', requireAdmin, async (req, res) => {
  try {
    const { minReferrals, minShares, creationFee, reachFeePercent, maxDeclineBeforeBan, declineBanDays } = req.body;
...
router.put('/campaigns/:id', requireAdmin, async (req, res) => {
  try {
    ...
    const campaign = await UserCampaign.findById(req.params.id).populate('creatorId', 'username email ib');
    if (!campaign) return res.status(404).json({ error: 'Campaign not found.' });
    if (campaign.adminStatus !== 'pending') return res.status(400).json({ error: 'Campaign already reviewed.' });
